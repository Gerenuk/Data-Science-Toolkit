...
class PeriodiPredict:
    def __init__(self, periodi_train, scorer_generator, exclude_bin_func):
        ...
        """
        self._scorers = {key: scorer_generator(bin_obj) for key, bin_obj in periodi_train.bin_dict.items()}
    ...
    def score(self, point):
        ...
        return self._score_agg(self._scorers[bin_key](data) for bin_key, data in bin_keys)

    def __repr__(self):
        return "\n".join(map(str, sorted(self._scorers.values(), key=lambda x: x._bin_score, reverse=True)))

    def get_top(self):
        result_dict = {key: (scorer, scorer(), key) for key, scorer in self._scorers.items()}
        result = []
        while result_dict:
            best_scorer, best_score, best_key = max(result_dict.values(), key=itemgetter(1))
