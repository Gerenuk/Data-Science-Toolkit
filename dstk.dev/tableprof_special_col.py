...
def pattern_parser(data, char_type_mapper, category_value_combiner_factory):
    ...
    for row in data:
        char_groups = [
            (chartype, list(chargroup))
            for chartype, chargroup in groupby(row, key=char_type_mapper)
        ]
        ...
        if char_signature not in pattern_result_map:
            pattern_result_map[char_signature] = tuple(
                category_value_combiner_factory[chartype]()
                for chartype in char_signature
            )
...
def result_formatter(
    pattern_counter, pattern_result_map, num_top_freq=2, join_sep=" / "
):
    ...
    for signature, num_pattern in most_freq_items:
        line = join_sep.join(
            combiner.result() for combiner in pattern_result_map[signature]
        )
...
class DistinctCombiner:
    ...
    def result(self):
        ...
        return "[{}*{}]".format(
            self.values.most_common(1)[0][0] + ".." if self.name is None else self.name,
            len(self.values),
        )
...
if __name__ == "__main__":
    ...
    filenames = list(
        glob.glob(
            r"Q:\SAS\advanced_analytics\prod\Projects\International Business Partners\Data collection\International Transactions (FCP)\*.csv"
        )
    )
    for filename in filenames:
        ...
        with open(filename) as f:
            ...
            for colname, coldata in cols.items():
                result = pattern_parser(
                    coldata, chartype_mapper, category_value_combiner
                )
