...
class ImageRows:
    ...
    def max_size(self):
        return (
            max(sum(image.x for image in row) for row in self.image_row_LL),
...
image_rows = ImageRows(
    {OrientedImage(Image.open(filename)) for filename in glob.glob(fileglob)}
)
while image_rows.has_images():
    first_image = max(
        [image.make_largest("x") for image in image_rows.remaining_images()],
        key=attrgetter("y"),
    )
