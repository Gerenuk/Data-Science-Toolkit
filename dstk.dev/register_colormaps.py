...
try:
    ...
    for cmap_type, modL in [
        ("seq", (palettable.colorbrewer.sequential, palettable.colorbrewer.diverging)),
        (
            "qual",
            (
        ...
        for mod in modL:
            for name in dir(mod):
                ...
                try:
                    ...
                    logging.warning(
                        "Failed to import colormap '{}' from {}".format(
                            name, mod.__name__
                        )
                    )
except ImportError as e:
    logging.warning(
        "Did not import colormaps from palettable due to import error: {}".format(e)
    )
...
try:
    ...
    for cmap_type, modL in [
        ("seq", (colorobject.custom_colormaps,)),
                ...
                try:
                    ...
                    if cmap_type == "seq":
                        ...
                        cmap = mpl.colors.ListedColormap(
                            [(r, g, b) for r, g, b, a in getattr(mod, name)]
                        )
                    mpl.cm.register_cmap(name, cmap)
                except AttributeError:
                    logging.warning(
    ...
    logging.warning(
        "Did not import colormaps from colorobject due to import error: {}".format(e)
    )
...
mpl.cm.register_cmap(
    "kelly", mpl.colors.ListedColormap(kelly_colors)
)  # check if remove black/white at beginning
