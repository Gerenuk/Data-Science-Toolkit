...
class AnonFirstLastName:
    def __init__(self, blacklist, firstnames, lastnames, firstlastnames, nonanon_regex=default_nonanon_regex):
        self.blacklist = blacklist
        self.firstnames = firstnames
        self.lastnames = lastnames
        self.firstlastnames = firstlastnames
        self.nonanon_regex = re.compile(nonanon_regex, flags=re.I)
        self.word_regex = re.compile(r"\b[^\W0-9_]{2,}\b")  # [^\W0-9_] effectively means "any letter", also Umlaut etc.
    ...
    def __call__(self, text):
        ...
        if not (names & self.firstnames.value and names & self.lastnames.value and len(names & self.firstlastnames.value) > 1):
...
def anon_firstlastname_spark(df, blacklist, firstnames, lastnames, *columns_to_anon):
    ...
    anonymizer = AnonFirstLastName(blacklist_bc, firstnames_bc, lastnames_bc, firstlastnames_bc)
    ...
    for colname in columns_to_anon:
        ...
        result = (result
                  .withColumn(colname + "_anonymized", anonymizer_udf(col(colname)))
                  .drop(colname)
                  )
