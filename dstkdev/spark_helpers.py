...
def countspark_top(
    ...
    ascending=False,
    maxcollect=1000,
    val_func=lambda row: 1,
    agg_func=add,
):
    result = (
        rdd.flatMap(lambda row: [(match, val_func(row)) for match in create_items(row)])
...
def countsparkdf_top(df, val_var, sum_var=None, ascending=False, maxcollect=1000):
    ...
    df3 = df2.orderBy(
        "count" if sum_var is None else "sum(" + sum_var + ")", ascending=ascending
    )
