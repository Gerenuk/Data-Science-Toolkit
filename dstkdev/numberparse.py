...
def counts_report(data):
    ...
    if len(most_common) <= max_display_values:
        print(
            "Distinct frequent values {}".format(
                ", ".join("{}".format(val) for val, _ in most_common)
            )
        )
        print(
            "Distinct ordered values {}".format(
                ", ".join("{}".format(val) for val, _ in sorted(most_common))
            )
        )
    ...
    if len(counts) <= 4:
        print(
            "Counts {}".format(
                ", ".join("{}({})".format(val, count) for val, count in most_common)
            )
        )
    else:
        ...
        print(
            "Counts {}".format(
                "{}({}), {}({}) .. {}({}), {}({})".format(
                    *itoo.chain.from_iterable(mostfreq + leastfreq)
                )
            )
        )
...
def size_report(data):
    ...
    if len(ordered) <= 4:
        print(
            "Counts {}".format(
                ", ".join("{}({})".format(val, counts[val]) for val in ordered)
            )
        )
    else:
        smallest = ordered[:2]
            ...
            "Ordered {}".format(
                "{}({}), {}({}) .. {}({}), {}({})".format(
                    *itoo.chain.from_iterable(
                        (val, counts[val]) for val in smallest + largest
                    )
                )
            )
        )
    ...
    try:
        print(
            "Quartiles {}".format(
                " - ".join(
                    str(np.percentile(data, percent, interpolation="nearest"))
...
class NumberParse:
    ...
    def report(self):
        ...
        print(
            "Parsed {} ({:.1%})".format(parsed_num, parsed_num / total_num),
            "; Unparsed {} ({:.1%})".format(unparsed_num, unparsed_num / total_num)
            if unparsed_num
            else "",
            sep="",
        )
