...
class FileParse:
    ...
    def parse_sep(self):
        ...
        self.sep_counts = {
            sep: Counter(counts) for sep, counts in all_charcounts.items()
        }
        self.best_sep = "".join(
            sep
            for sep, _count in sorted(
                self.sep_counts.items(),
                key=lambda t: max(q * r for q, r in t[1].items()),
                reverse=True,
            )
        )
        ...
        for sep in self.best_sep[:5]:
            print(
                "{} : {}".format(
                    ...
                    ", ".join(
                        "{}sep ({})".format(
                            num, count if count != self.linecount else "ALL"
                        )
                        for num, count in self.sep_counts[sep].most_common(5)
                    ),
                )
            )
    ...
    def parse_rows(self, sep=None):
        ...
        print(
            ...
            ", ".join(
                "{}length({})".format(
                    length, count if count != self.linecount else "ALL"
                )
                for length, count in row_lengths.most_common()
            ),
        )
...
if __name__ == "__main__":
    fp = FileParse(
        r"F:\V\VT_Oberursel\RBV\KA\CRM_DM_Mafo\04_CMI\02_Aufgaben\04_Datenquellen\12_SAP_Reiseaufträge, Kundendaten\RA_Abzug_ab20110101_ALL_lines10000.csv"
    )
    fp.parse_sep()
    fp.parse_rows()
