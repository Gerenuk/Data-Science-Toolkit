...
class TimeseriesModel:
    ...
    def fit_predict_multiple(self, ts, pred_start, pred_end, step=1, length=None,fit_params=None, pred_params=None):
        if length is None:
            length=step
        ...
        for i in range(0, len(dates)-length, step):
            ...
            pred=self.fit_predict(ts, iter_pred_start, iter_pred_end, fit_params=fit_params, pred_params=pred_params)
            results.append(pred)
...
class Prophet(TimeseriesModel):
    def __init__(self, **model_params):
        """
        model_params: https://github.com/facebook/prophet/blob/master/python/fbprophet/forecaster.py
        """
        self.model_params=model_params
    ...
    def predict(self, pred_start, pred_end, show_warnings=False, **pred_params):
        with warnings.catch_warnings():
            ...
            df_pred = self.model_.predict(pd.DataFrame({"ds": pd.date_range(pred_start, pred_end, freq=self._inferred_freq)})) # freq not set here
